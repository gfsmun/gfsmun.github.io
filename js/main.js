/* ==========================================================================
   GFSMUN - main.js
   Navigation, scroll reveals, lightbox, forms, utilities
   Vanilla JS, no dependencies, deferred
   ========================================================================== */
"use strict";

/* --------------------------------------------------------------------------
   0. LETTER EXPAND - mobile collapsible body text
   -------------------------------------------------------------------------- */
(function initLetterExpand() {
  const btn = document.querySelector(".letter__expand");
  const text = document.getElementById("sgText");
  if (!btn || !text) return;
  // Only collapsible on mobile - but JS works everywhere, CSS hides button on desktop
...
(function initContactForm() {
  ...
  function setError(field, show) {
    if (show) field.classList.add("form-field--error");
  ...
  form.addEventListener("submit", (e) => {
    ...
    // Simulate success - no external endpoint
    ...
    setTimeout(() => {
      if (status) {
        ...
        status.textContent = "Thank you - your message has been received. The Secretariat will reply within 24–48 hours.";
      }
      form.reset();
      fields.forEach((f) => f.classList.remove("form-field--error"));
      if (btn) {
        btn.disabled = false;
...
/* --------------------------------------------------------------------------
   6. IMAGE FALLBACK - GRACEFUL PLACEHOLDER FOR MISSING ASSETS
   -------------------------------------------------------------------------- */
  ...
  document.querySelectorAll("img").forEach((img) => {
    img.addEventListener("error", () => {
      ...
      if (container.classList.contains("gallery-item") ||
          ...
          container.classList.contains("timeline__card-media") ||
